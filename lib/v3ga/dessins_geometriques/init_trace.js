...
function init_(w, h, opts) {
  if (opts.svg === true) {
    ...
    if (canvasElmt && canvasElmt.parentNode) canvasElmt.parentNode.removeChild(canvasElmt);
    canvasElmt = null;

    const stray = document.querySelector("canvas");
    ...
    if (svgElmt && svgElmt.parentNode) svgElmt.parentNode.removeChild(svgElmt);
    ...
    svgElmt.style.width  = `${w}px`;
    svgElmt.style.height = `${h}px`;
    svgElmt.style.display = "block";

    (document.getElementById("sketchHost") || document.body).appendChild(svgElmt);
    ...
    let r = createCanvas(w, h);
    canvasElmt = r?.elt || document.querySelector("canvas");

    const host = document.getElementById("sketchHost");
    if (canvasElmt && host) host.appendChild(canvasElmt);
  }
